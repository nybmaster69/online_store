...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # {'items': [OrderedDict([('product', < Product: Product 1 >), ('quantity', 5)]),
        #            OrderedDict([('product', < Product: phone >), ('quantity', 3)])], 'total_cost': 0,
    ...
    def to_representation(self, instance):
        ...
        rep['products'] = OrderProductSerializer(OrderProduct.objects.filter(
            order=instance.id
        ), many=True).data
        return rep
